...
public class CustomEventMaker {
	 ...
	 * @param obj custom return parameter(cannot be null)
	 * @param clazz class of the event
	 * @return a new {@link Event<T>}
	 * @param <T> {@link Object}
	 * @param <R> {@link Object}
	 */
	public static <T, R> Event<T> createEvent(@NotNull R obj, Class<T> clazz) {
		return EventFactory.of(listeners -> (T) Proxy.newProxyInstance(EventFactory.class.getClassLoader(), new Class[]{clazz}, (proxy, method, args) -> {
			for (var listener : listeners) {
				float value = Objects.requireNonNull(EventFactoryInvoker.invokeMethod(listener, method, args));
				if (value < 0.0f) {
					value = 0;
				}
				return value;
